"""Storage-agnostic test-writing tool loop: given a raw requirement, drives
an LLM through rag_search / web_search / write_code until it has produced
pytest test file(s), validating each one before accepting it.

This is shared by two callers that persist an accepted file differently:
- app/agent_loop.py's local fallback path (Server A unreachable) - writes
  into a session's workspace directory on disk.
- server_a/main.py's /generate-requirement endpoint - collects files into
  an in-memory dict to return over HTTP, no disk I/O.

That's what `write_file` (a callback, not a fixed disk path) is for. Note
this does NOT extend to test validation: `validate_test_file_before_freeze`
below always writes its own real, throwaway temp files and runs a real
(Docker-sandboxed) pytest subprocess against them, regardless of where the
caller's `write_file` ultimately persists the accepted content - so whatever
process calls `run_test_writer_loop` still needs Docker + the sandbox image
available locally.
"""
...
from app.config import TEST_GEN_VALIDATION_TIMEOUT
...
Before calling `write_code`, honestly assess your own understanding: do you
actually know, with certainty, the correct expected value or behavior for
every assertion you're about to write - a hand-computed numeric result,
which exception a standard library operation raises, the correct behavior of
a named algorithm, a domain fact the requirement assumes you know? If you are
not certain, use `rag_search` and/or `web_search` to resolve it yourself -
you decide how many times and with what queries, including searching again
after a result that didn't fully answer your question. Only call `write_code`
once you're not just done searching, but actually confident - if you were
already certain to begin with, searching first isn't required. These test
files are frozen the moment they're written - the implementation phase can
never edit them later, so a wrong expected value here is a bug nothing
downstream can fix, no matter how correct the implementation is. Searching
costs one extra tool call; writing a wrong assertion costs the entire run.
...
def validate_test_file_before_freeze(test_source: str, module_hint: str) -> ValidationResult:
    """Gate a test file before it's frozen (see run_test_writer_loop) so a
    ...
    it's permanently locked in, instead of only discovered after the
    implementation phase burns its whole iteration budget against an
    unpassable frozen test.
    ...
    Always creates its own real temp directory and runs a Docker-sandboxed
    subprocess there (via app.tools.run_command_in_directory) - this is
    independent of wherever a subsequently-accepted file gets persisted, so
    whichever process calls this (Server A or Server B's local fallback)
    needs Docker + the sandbox image available locally.
    """
    try:
...
def _dispatch_test_writer_tool_call(
    ...
    rag_search_fn: Callable[..., str],
    web_search_fn: Callable[..., str],
    write_file: Callable[[str, str], Dict[str, Any]],
) -> Any:
    """Execute a single test-writer tool call and return its raw result
    (never raises)."""
    if tool_name == "rag_search":
        ...
        try:
            ...
            top_k = 5
        return rag_search_fn(query=query, top_k=top_k)
    ...
    if tool_name == "web_search":
        ...
        try:
            ...
            top_k = 5
        return web_search_fn(query=query, top_k=top_k)
...
def run_test_writer_loop(
    client: OpenAI,
    model_name: str,
    num_ctx: int,
    requirement: str,
    write_file: Callable[[str, str], Dict[str, Any]],
    rag_search: Optional[Callable[..., str]] = None,
    web_search: Optional[Callable[..., str]] = None,
) -> Tuple[List[str], Dict[str, str], List[Dict[str, Any]]]:
    """Ask the LLM to write test file(s) from the requirement alone.

    `write_file(filepath, content) -> {"success": bool, "path": str,
    "error": str | None}` decides where an accepted file actually ends up -
    session-workspace disk (Server B's local fallback) or an in-memory dict
    (Server A's /generate-requirement). `rag_search`/`web_search` default to
    `app.tools.rag_search`/`app.tools.web_search` (resolved dynamically at
    call time, not bound at import time, so monkeypatching those stays
    effective) when not given an override.
    ...
    files, and the caller decides how to handle that.
    """
    rag_search_fn = rag_search if rag_search is not None else app_tools.rag_search
    web_search_fn = web_search if web_search is not None else app_tools.web_search
    ...
    for iteration in range(1, _TEST_GEN_MAX_ITERATIONS + 1):
        ...
        for tool_call in tool_calls:
            ...
            if tool_name == "write_code":
                ...
                # Validate BEFORE this ever gets persisted / gets frozen: a
                # test that's structurally broken (doesn't import an
                # implementation module) or would fail regardless of the
                # implementation (e.g. a name used but never imported) can
                # never be fixed later once frozen - the implementation
                # phase would just burn its whole iteration budget against
                # an unpassable test, the failure mode behind repeated real
                # max_iterations_reached runs. See
                # validate_test_file_before_freeze for the two validation
                # stages.
            ...
            result = _dispatch_test_writer_tool_call(tool_name, arguments, rag_search_fn, web_search_fn, write_file)
            trace.append(
                {
                    "phase": "test_generation",
                    "iteration": iteration,
